...
function xmldb_local_customerintel_upgrade($oldversion) {
    ...
    // Upgrade to version 2025203016 - Add diagnostics and enhanced trace intelligence
    if ($oldversion < 2025203016) {
        // ============================================================
        // CREATE local_ci_diagnostics table for Run Doctor functionality
        // ============================================================
        $table = new xmldb_table('local_ci_diagnostics');
        
        if (!$dbman->table_exists($table)) {
            debugging("Creating local_ci_diagnostics table (version 2025203016)", DEBUG_NORMAL);
            
            // Define fields
            $table->add_field('id', XMLDB_TYPE_INTEGER, '10', null, XMLDB_NOTNULL, XMLDB_SEQUENCE);
            $table->add_field('runid', XMLDB_TYPE_INTEGER, '10', null, XMLDB_NOTNULL);
            $table->add_field('status', XMLDB_TYPE_CHAR, '20', null, XMLDB_NOTNULL, null, 'OK');
            $table->add_field('summary', XMLDB_TYPE_TEXT, 'small', null, null);
            $table->add_field('issues_count', XMLDB_TYPE_INTEGER, '10', null, XMLDB_NOTNULL, null, '0');
            $table->add_field('warnings_count', XMLDB_TYPE_INTEGER, '10', null, XMLDB_NOTNULL, null, '0');
            $table->add_field('diagnostics_data', XMLDB_TYPE_TEXT, 'big', null, XMLDB_NOTNULL);
            $table->add_field('timecreated', XMLDB_TYPE_INTEGER, '10', null, XMLDB_NOTNULL, null, '0');
            $table->add_field('timemodified', XMLDB_TYPE_INTEGER, '10', null, null, null, '0');
            ...
            // Add indexes
            $table->add_index('runid_idx', XMLDB_INDEX_UNIQUE, ['runid']);
            ...
            debugging("SUCCESS: Created local_ci_diagnostics table", DEBUG_NORMAL);
        } else {
            debugging("local_ci_diagnostics table already exists - skipping creation", DEBUG_NORMAL);
        }
        
        // Add enhanced trace intelligence settings
        if (get_config('local_customerintel', 'enable_detailed_trace_logging') === false) {
            set_config('enable_detailed_trace_logging', '0', 'local_customerintel');
            debugging("Initialized enable_detailed_trace_logging setting to 0 (disabled by default)", DEBUG_NORMAL);
        }
        
        if (get_config('local_customerintel', 'enable_run_doctor') === false) {
            set_config('enable_run_doctor', '1', 'local_customerintel');
            debugging("Initialized enable_run_doctor setting to 1 (enabled by default)", DEBUG_NORMAL);
        }
        
        if (get_config('local_customerintel', 'auto_run_diagnostics') === false) {
            set_config('auto_run_diagnostics', '1', 'local_customerintel');
            debugging("Initialized auto_run_diagnostics setting to 1 (enabled by default)", DEBUG_NORMAL);
        }
        
        debugging("Enhanced Trace Intelligence & Run Doctor: Upgrade completed successfully", DEBUG_NORMAL);
        
        upgrade_plugin_savepoint(true, 2025203016, 'local', 'customerintel');
    }

    // Milestone 0: Interactive Intelligence Cache Manager
    if ($oldversion < 2025203020) {
        $table = new xmldb_table('local_ci_run');
        $field = new xmldb_field('cache_strategy', XMLDB_TYPE_CHAR, '20', null, null, null, null, 'mode');
        if (!$dbman->field_exists($table, $field)) {
            $dbman->add_field($table, $field);
            debugging("Milestone 0: Added cache_strategy field to local_ci_run", DEBUG_NORMAL);
        }
        upgrade_plugin_savepoint(true, 2025203020, 'local', 'customerintel');
    }

    // Fix: Add missing local_ci_diagnostics table
    if ($oldversion < 2025203022) {
        $table = new xmldb_table('local_ci_diagnostics');
        ...
        if (!$dbman->table_exists($table)) {
            ...
            $dbman->create_table($table);
            debugging("Created missing local_ci_diagnostics table", DEBUG_NORMAL);
        }

        upgrade_plugin_savepoint(true, 2025203022, 'local', 'customerintel');
    }

    // Milestone 1: Per-company NB caching
    if ($oldversion < 2025203023) {

        // Define table local_ci_nb_cache
        $table = new xmldb_table('local_ci_nb_cache');

        // Add fields
        $table->add_field('id', XMLDB_TYPE_INTEGER, '10', null, XMLDB_NOTNULL, XMLDB_SEQUENCE, null);
        $table->add_field('company_id', XMLDB_TYPE_INTEGER, '10', null, XMLDB_NOTNULL, null, null);
        $table->add_field('nbcode', XMLDB_TYPE_CHAR, '10', null, XMLDB_NOTNULL, null, null);
        $table->add_field('jsonpayload', XMLDB_TYPE_TEXT, 'long', null, XMLDB_NOTNULL, null, null);
        $table->add_field('citations', XMLDB_TYPE_TEXT, 'long', null, null, null, null);
        $table->add_field('version', XMLDB_TYPE_INTEGER, '10', null, XMLDB_NOTNULL, null, '1');
        $table->add_field('timecreated', XMLDB_TYPE_INTEGER, '10', null, XMLDB_NOTNULL, null, null);

        // Add keys
        $table->add_key('primary', XMLDB_KEY_PRIMARY, ['id']);
        $table->add_key('company_fk', XMLDB_KEY_FOREIGN, ['company_id'], 'local_ci_company', ['id']);

        // Add indexes
        $table->add_index('company_nb_version', XMLDB_INDEX_UNIQUE, ['company_id', 'nbcode', 'version']);
        $table->add_index('timecreated_idx', XMLDB_INDEX_NOTUNIQUE, ['timecreated']);

        // Create table if it doesn't exist
        if (!$dbman->table_exists($table)) {
            $dbman->create_table($table);
            debugging("Milestone 1: Created local_ci_nb_cache table for per-company NB caching", DEBUG_NORMAL);
        }

        upgrade_plugin_savepoint(true, 2025203023, 'local', 'customerintel');
    }

    // Milestone 1 Task 2: Prompt config scaffolding
    if ($oldversion < 2025203024) {

        // Add prompt_config field to local_ci_run
        $table = new xmldb_table('local_ci_run');
        $field = new xmldb_field('prompt_config', XMLDB_TYPE_TEXT, null, null, null, null, null);

        if (!$dbman->field_exists($table, $field)) {
            $dbman->add_field($table, $field);
        }

        // Add refresh_config field to local_ci_run
        $field = new xmldb_field('refresh_config', XMLDB_TYPE_TEXT, null, null, null, null, null);

        if (!$dbman->field_exists($table, $field)) {
            $dbman->add_field($table, $field);
        }

        upgrade_plugin_savepoint(true, 2025203024, 'local', 'customerintel');
    }
