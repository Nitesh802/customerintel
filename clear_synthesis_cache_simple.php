<?php
/**
 * Emergency: Clear ALL Synthesis Cache
 * Deletes all cached synthesis to force regeneration
 *
 * Usage: https://sales.multi.rubi.digital/local/customerintel/clear_synthesis_cache_simple.php
 */
...
echo "<!DOCTYPE html><html><head><title>Clear ALL Synthesis Cache</title>";
echo "<style>
...
echo "<h1>🗑️ Clear ALL Synthesis Cache - Emergency Fix</h1>";
echo "<hr>";
...
if (!$confirmed) {
    ...
    echo "<h2>⚠️ EMERGENCY: Clear All Cached Synthesis</h2>";
    echo "<p><strong>Problem:</strong> Run 241 failed because cached synthesis had wrong NB code format</p>";
    echo "<p><strong>Solution:</strong> Delete ALL synthesis cache records to force fresh generation</p>";
    echo "</div>";

    echo "<div class='box warning'>";
    echo "<h2>⚠️ Warning: This will delete ALL synthesis cache</h2>";
    echo "<p>This action will:</p>";
    echo "<ul>";
    echo "<li>Delete ALL records from <code>local_ci_synthesis</code> table</li>";
    echo "<li>Force next run to regenerate synthesis from scratch</li>";
    echo "<li>Take 5-10 minutes to rebuild synthesis for each run</li>";
    echo "</ul>";
    echo "<p><strong>This cannot be undone!</strong></p>";
    echo "<form method='get'>";
    echo "<input type='hidden' name='confirm' value='1'>";
    echo "<button type='submit'>Yes, Clear ALL Synthesis Cache</button>";
    echo "</form>";
    echo "</div>";
    ...
    echo "<p>Total synthesis cache records: <strong>{$cache_count}</strong></p>";

    if ($cache_count > 0) {
        $records = $DB->get_records('local_ci_synthesis', [], 'createdat DESC', 'id, runid, createdat', 0, 10);
        ...
        foreach ($records as $record) {
            $age = round((time() - $record->createdat) / 3600, 1);
            echo "<tr>";
            echo "<td style='padding:8px; border:1px solid #ddd;'>{$record->id}</td>";
            echo "<td style='padding:8px; border:1px solid #ddd;'>Run {$record->runid}</td>";
            echo "<td style='padding:8px; border:1px solid #ddd;'>" . date('Y-m-d H:i:s', $record->createdat) . "</td>";
    ...
    try {
        ...
        echo "<p>Next steps:</p>";
        echo "<ol>";
        echo "<li>Clear opcache: Run <a href='clear_opcache.php'>clear_opcache.php</a></li>";
        echo "<li>Go to Dashboard</li>";
        echo "<li>Create Run 242 with 'Full Refresh'</li>";
        echo "<li>Wait for cron to process (5-10 minutes)</li>";
        echo "<li>System will regenerate synthesis with correct NB code format</li>";
