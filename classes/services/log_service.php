...
class log_service {
    ...
    private static function write($runid, $level, $message) {
        ...
        try {
            ...
            // Log to error_log (not displayed on page, only in logs)
            error_log("[{$level}] Run {$runid}: {$message}");

            // Only use mtrace in CLI mode, not web mode
            if (CLI_SCRIPT) {
                mtrace("[{$level}] Run {$runid}: {$message}");
            }
        } catch (\Exception $e) {
            // If logging fails, log to error_log
            error_log("Failed to write log: " . $e->getMessage());

            // Only mtrace in CLI mode
            if (CLI_SCRIPT) {
