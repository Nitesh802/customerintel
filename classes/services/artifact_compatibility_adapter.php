...
class artifact_compatibility_adapter {
    ...
    private static $artifact_aliases = [
        ...
        'canonical_nb_dataset' => 'canonical_dataset'
    ];
    ...
    private static $schema_transformations = [
        ...
        'canonical_dataset' => [
            'ensure_fields' => ['metadata', 'nb_data', 'citations', 'processing_stats'],
            'validate_structure' => true,
            'extract_nb_keys' => true
        ]
    ];
    ...
    public function save_synthesis_bundle($runid, $result) {
        try {
            ...
            // CREATE synthesis record if it doesn't exist
            if (!$synthesis) {
                log_service::info($runid, "[Compatibility] Creating new synthesis record");

                // Load run data to get company IDs for M1T3 metadata
                $run = $this->db->get_record('local_ci_run', ['id' => $runid]);

                $synthesis = new \stdClass();
                $synthesis->runid = $runid;
                $synthesis->htmlcontent = $result['html'] ?? '';
                $synthesis->jsoncontent = json_encode([]);  // Will be filled below
                $synthesis->voice_report = $result['voice_report'] ?? '';
                $synthesis->selfcheck_report = $result['selfcheck_report'] ?? '';
                $synthesis->qa_report = $result['qa_report'] ?? '';
                $synthesis->coherence_report = $result['coherence_report'] ?? '';
                $synthesis->pattern_alignment_report = $result['pattern_alignment_report'] ?? '';

                // M1T3: Enhanced metadata - include source and target company IDs
                if ($run) {
                    $synthesis->source_company_id = $run->companyid;
                    $synthesis->target_company_id = $run->targetcompanyid ?? null;
                }

                $synthesis->createdat = time();
                $synthesis->updatedat = time();

                $synthesis->id = $this->db->insert_record('local_ci_synthesis', $synthesis);

                log_service::info($runid,
                    "[Compatibility] Synthesis record created (ID: {$synthesis->id}) with M1T3 metadata");
            }

            // UPDATE synthesis record with cache structure
            ...
            $synthesis->htmlcontent = $result['html'] ?? $synthesis->htmlcontent;
            $synthesis->updatedat = time();

            $this->db->update_record('local_ci_synthesis', $synthesis);

            // NOTE: Sections are stored in jsoncontent->synthesis_cache structure
            // The local_ci_synthesis_section table doesn't exist in the schema,
            // so we store sections within the synthesis JSON instead
    ...
    private function detect_phase($artifact_name) {
        $phase_map = [
            ...
            'canonical_nb_dataset' => 'synthesis',
            'qa_scores' => 'qa',
    ...
    private function build_complete_cache_structure($result, $runid) {
        /**
         * SYNTHESIS METADATA ENHANCEMENT - M1 Task 3 (Nov 2025)
         *
         * This enhancement addresses Jon's requirement:
         * "Synthesis results are always keyed and cached using both the source and target ID"
         *
         ...
         * - cache_source with validation that both IDs match when reused
         */

        // Collect enhanced metadata for provenance and cache validation
        $enhanced_metadata = $this->build_enhanced_metadata($runid);

        $cache_structure = [
            'version' => 'v17.1-unified-compatibility',
            'built_at' => time(),
            'compatibility_adapter' => self::COMPATIBILITY_VERSION,
            'metadata' => $enhanced_metadata,  // M1 Task 3: Enhanced metadata
        ...
        log_service::info($runid,
            "[Compatibility] Built complete cache structure with v17.1 unified compatibility and enhanced metadata (M1T3)");
    ...
    /**
     * Build enhanced metadata structure - M1 Task 3 (Nov 2025)
     *
     * Creates complete metadata structure with:
     * - Explicit source_company_id and target_company_id (Jon's requirement)
     * - Composite synthesis_key for dual-key tracking
     * - Model and prompt configuration for reproducibility
     * - Cache source metadata with validation
     * - Section count and timing information
     *
     * @param int $runid Run ID
     * @return array Enhanced metadata structure
     */
    private function build_enhanced_metadata($runid) {
        // Load run record with company IDs
        $run = $this->db->get_record('local_ci_run',
            ['id' => $runid],
            'id, companyid, targetcompanyid, prompt_config, reusedfromrunid, timecreated',
            MUST_EXIST
        );

        // Create explicit synthesis key using source + target IDs (Jon's requirement #1)
        ...
        // Count sections from synthesis_final_bundle artifact
        $section_count = $this->get_section_count($runid);

        // Build complete metadata structure
        $metadata = [
            'runid' => $runid,
            'source_company_id' => (int)$run->companyid,
            'target_company_id' => (int)$run->targetcompanyid,
            'synthesis_key' => $synthesis_key,
            'model_used' => 'gpt-4',  // Default model - can be enhanced to track actual model
            'prompt_config' => $prompt_config,
            'section_count' => $section_count,
            'timecreated' => time(),
            'cache_source' => $this->get_cache_source_metadata($run)
        ];

        log_service::info($runid,
            "[M1T3-Metadata] Enhanced metadata built: synthesis_key={$synthesis_key}, " .
    ...
    /**
     * Get cache source metadata including validation that both source and target match
     * Addresses Jon's requirement: synthesis keyed by both source AND target ID
     *
     * @param object $run Current run object
     * @return array Cache source metadata
     */
    private function get_cache_source_metadata($run) {
        ...
        $cached_run = $this->db->get_record('local_ci_run',
        ...
        // Verify both source AND target match (Jon's requirement #1)
        ...
        log_service::info($run->id,
            "[M1T3-CacheValidation] Cached from run {$run->reusedfromrunid}: " .
    ...
    /**
     * Get section count from synthesis artifacts
     *
     * @param int $runid Run ID
     * @return int Section count
     */
    private function get_section_count($runid) {
        // Try to get from synthesis_final_bundle artifact
        $artifact = $this->db->get_record('local_ci_artifact', [
            'runid' => $runid,
            'phase' => 'synthesis',
        ...
        if ($artifact && !empty($artifact->jsondata)) {
            $bundle_data = json_decode($artifact->jsondata, true);
            if (json_last_error() === JSON_ERROR_NONE) {
                if (isset($bundle_data['section_count'])) {
                    return (int)$bundle_data['section_count'];
                }
            }
        }
