...
class artifact_repository {
     ...
     * @param bool $force_save Force save even if trace mode is disabled (for critical artifacts)
     * @return bool Success status
     */
    public function save_artifact($runid, $phase, $artifacttype, $data, $force_save = false) {
        try {
            // Check if trace mode is enabled (unless forcing save for critical artifacts)
            if (!$force_save && !$this->is_trace_mode_enabled()) {
    ...
    /**
     * Append trace entry for pipeline transparency
     *
     * Appends a trace entry to the pipeline trace artifact.
     * Creates a new trace artifact if it doesn't exist.
     *
     * @param int $runid Run identifier
     * @param array $trace_entry Trace entry data
     * @return bool Success status
     */
    public function append_trace($runid, $trace_entry) {
        try {
            // Get existing trace artifact if it exists (returns DB record)
            $existing_record = $this->get_artifact($runid, 'trace', 'pipeline_trace');

            $trace_data = [];
            if ($existing_record && !empty($existing_record->jsondata)) {
                // Decode the JSON data
                $existing_data = json_decode($existing_record->jsondata, true);
                if ($existing_data && isset($existing_data['entries'])) {
                    $trace_data = $existing_data['entries'];
                }
            }

            // Append new entry
            $trace_data[] = array_merge($trace_entry, [
                'timestamp' => microtime(true),
                'time' => date('Y-m-d H:i:s')
            ]);

            // Save updated trace
            return $this->save_artifact($runid, 'trace', 'pipeline_trace', [
                'entries' => $trace_data,
                'count' => count($trace_data)
            ], true); // Force save even if trace mode is disabled

        } catch (\Exception $e) {
            debugging("Failed to append trace entry: " . $e->getMessage(), DEBUG_DEVELOPER);
            return false;
        }
    }
}