...
require_once(__DIR__ . '/nb_cache_service.php');
...
class nb_orchestrator {
    ...
    public function execute_nb(int $runid, string $nbcode): array {
        ...
        // Milestone 1: Check cache for source company NBs (NB-1 through NB-7)
        // Milestone 1: Check cache for target company NBs (NB-8 through NB-15)
        // Read cache strategy from run configuration
        $force_refresh = isset($run->cache_strategy) && $run->cache_strategy === 'force_refresh';
        $cache_hit = false;
        $cached_payload = null;
        $cached_citations = null;

        // Determine which company this NB belongs to
        $nb_number = (int)str_replace(['NB', 'NB-'], '', $nbcode);
        $cache_company_id = ($nb_number >= 1 && $nb_number <= 7) ? $run->companyid : $run->targetcompanyid;
        ...
        if (!$force_refresh) {
            // Check if refresh_config forces regeneration
        ...
        // Only check cache if we have a valid company ID and not forcing refresh
        if ($cache_company_id && !$force_refresh) {
            $cache = nb_cache_service::get_cached_nb($cache_company_id, $nbcode);
            if ($cache) {
                $cache_hit = true;
                $cached_payload = json_decode($cache->jsonpayload, true);
                $cached_citations = $cache->citations ? json_decode($cache->citations, true) : [];

                $this->log_to_moodle($runid, $nbcode, "Cache HIT for company {$cache_company_id}");
                debugging("Milestone 1: Cache HIT for {$nbcode} company {$cache_company_id}", DEBUG_DEVELOPER);
                ...
                // Save cached result to database (Bug #9 fix)
                // This ensures local_ci_nb_result has records even for cache hits
                try {
                    $this->save_nb_result($runid, $nbcode, $cached_result);
                    $this->log_to_moodle($runid, $nbcode, "Cache hit result saved to database");
                } catch (\Exception $e) {
                    // Log error but don't fail - cached result is still valid
                    $this->log_to_moodle($runid, $nbcode, "Failed to save cached result to database: " . $e->getMessage());
                    debugging("execute_nb(): Failed to save cached {$nbcode} to database: " . $e->getMessage(), DEBUG_DEVELOPER);
                }

                return $cached_result;
            } else {
                $this->log_to_moodle($runid, $nbcode, "Cache MISS for company {$cache_company_id}");
                debugging("Milestone 1: Cache MISS for {$nbcode} company {$cache_company_id}", DEBUG_DEVELOPER);
            }
        }
        ...
        // Milestone 1: Store in cache if not from cache
        if (!$cache_hit && $cache_company_id) {
            try {
                $cache_id = nb_cache_service::store_nb(
                    $cache_company_id,
                    $nbcode,
                    json_encode($validpayload),
                    json_encode($citations)
                );

                if ($cache_id) {
                    $this->log_to_moodle($runid, $nbcode, "Stored in cache (ID: {$cache_id}) for company {$cache_company_id}");
                    debugging("Milestone 1: Stored {$nbcode} in cache (ID: {$cache_id}) for company {$cache_company_id}", DEBUG_DEVELOPER);
                } else {
                    $this->log_to_moodle($runid, $nbcode, "Failed to store in cache for company {$cache_company_id}");
                }
            } catch (\Exception $e) {
                // Log error but don't fail the NB execution
                $this->log_to_moodle($runid, $nbcode, "Cache store error: " . $e->getMessage());
                debugging("Milestone 1: Cache store error for {$nbcode}: " . $e->getMessage(), DEBUG_DEVELOPER);
            }
        }
        ...
        // Save result to database (Bug #8 fix)
        try {
            $this->save_nb_result($runid, $nbcode, $result);
        } catch (\Exception $e) {
            // Log error but don't fail - result is still valid
            $this->log_to_moodle($runid, $nbcode, "Failed to save result to database: " . $e->getMessage());
            debugging("execute_nb(): Failed to save {$nbcode} to database: " . $e->getMessage(), DEBUG_DEVELOPER);
        }
    ...
    protected function execute_nb_with_real_apis(int $runid, string $nbcode, \stdClass $company, string $openaiapikey, string $perplexityapikey, ?\stdClass $targetcompany = null): array {
        ...
        // Milestone 1: Check cache before API calls
        $run = $DB->get_record('local_ci_run', ['id' => $runid]);
        $force_refresh = isset($run->cache_strategy) && $run->cache_strategy === 'force_refresh';

        // Determine which company this NB belongs to
        // Per Milestone 1 spec: NB-1 to NB-7 → source, NB-8 to NB-15 → target
        // Extract number from nbcode (e.g., 'NB-1' → 1, 'NB-15' → 15)
        ...
        if ($nb_number >= 1 && $nb_number <= 7) {
            ...
            // NB-8 to NB-15: Cache under target company
            $cache_company_id = $targetcompany->id;
        } else {
            // Fallback for single-entity runs or invalid NB numbers
            $cache_company_id = $company->id;
        }
        ...
        if ($cache_company_id && !$force_refresh) {
            ...
            if ($cache) {
                debugging("Milestone 1: Cache HIT for {$nbcode} company {$cache_company_id}", DEBUG_DEVELOPER);
                \local_customerintel\services\log_service::info($runid, "Cache HIT for {$nbcode}");

                $duration = (microtime(true) - $start_time) * 1000;
                return [
                    'nbcode' => $nbcode,
                    'payload' => json_decode($cache->jsonpayload, true),
                    'citations' => $cache->citations ? json_decode($cache->citations, true) : [],
                    'duration_ms' => $duration,
                    'tokens_used' => 0,
                    'status' => 'completed',
                    'cache_hit' => true
                ...
                debugging("Milestone 1: Cache MISS for {$nbcode} company {$cache_company_id}", DEBUG_DEVELOPER);
                \local_customerintel\services\log_service::info($runid, "Cache MISS for {$nbcode}");
            }
        }
        ...
        // Milestone 1: Store in cache after successful generation
        if ($cache_company_id) {
            try {
                $cache_id = nb_cache_service::store_nb(
                    $cache_company_id,
                ...
                if ($cache_id) {
                    debugging("Milestone 1: Stored {$nbcode} in cache (ID: {$cache_id}) for company {$cache_company_id}", DEBUG_DEVELOPER);
                    \local_customerintel\services\log_service::info($runid, "Stored {$nbcode} in cache (ID: {$cache_id})");
                } else {
                    \local_customerintel\services\log_service::warning($runid, "Failed to store {$nbcode} in cache for company {$cache_company_id}");
                }
            } catch (\Exception $e) {
                // Log error but don't fail the NB execution
                \local_customerintel\services\log_service::error($runid, "Cache store error for {$nbcode}: " . $e->getMessage());
                debugging("Milestone 1: Cache store error for {$nbcode}: " . $e->getMessage(), DEBUG_DEVELOPER);
            }
        }
    ...
    protected function load_nb_schema(string $nbcode): array {
        ...
        // Convert NB-1 format to nb1 format (remove hyphen, lowercase)
        $filename = str_replace('-', '', strtolower($nbcode));
        $schemafile = $CFG->dirroot . '/local/customerintel/schemas/' . $filename . '.json';
    ...
    public function execute_full_protocol($run): bool {
        ...
        try {
            ...
            if ($final_status === 'completed') {
                try {
                    // Step 1: Assemble report sections
                    ...
                    // Step 2: Build synthesis report automatically (ensures synthesis runs on initial execution)
                    \local_customerintel\services\log_service::info($run->id, "Starting automatic synthesis generation for run {$run->id}");
                    require_once(__DIR__ . '/synthesis_engine.php');
                    $synthesis_engine = new \local_customerintel\services\synthesis_engine();
                    $synthesis_bundle = $synthesis_engine->build_report($run->id, false);
                    \local_customerintel\services\log_service::info($run->id, "Synthesis generated automatically for run {$run->id}");

                } catch (\Exception $e) {
                    \local_customerintel\services\log_service::error($run->id, "Report assembly/synthesis failed: " . $e->getMessage());
                    // Log full trace for debugging
                    \local_customerintel\services\log_service::debug($run->id, "Exception trace: " . $e->getTraceAsString());
                }
            }
