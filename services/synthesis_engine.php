...
class synthesis_engine {
    ...
    public function get_cached_synthesis($runid): ?array {
        ...
        // Check cache version
        if (!isset($cache['version']) || $cache['version'] !== 'v2-citations-inline') {
            return null;
        }
        
        // Check if cache is recent (within 24 hours)
        if (isset($cache['built_at']) && time() - $cache['built_at'] > 86400) {
            return null;
        }
        
        // Reconstruct result from cache
        if (isset($cache['render'])) {
            return [
    ...
    public function cache_synthesis($runid, $result): void {
        ...
        // Update cache
        $json_data['synthesis_cache'] = [
            'version' => 'v2-citations-inline',
            'built_at' => time(),
        ...
        try {
            ...
            if ($artifact && !empty($artifact->jsondata)) {
                ...
                if (json_last_error() === JSON_ERROR_NONE && isset($data['normalized_citations'])) {
                    debugging("Using normalized citation artifact for run {$runid}", DEBUG_DEVELOPER);
                    return $data;
                }
            }
            
            debugging("No valid normalized citation artifact found for run {$runid}, using database fallback", DEBUG_DEVELOPER);
            return null;
