...
if ($runid > 0) {
    ...
    $trace_enabled = get_config('local_customerintel', 'enable_detailed_trace_logging') === '1';
    ...
    foreach ($runs as $run) {
        ...
        $rundata->status_processing = ($run->status === 'processing'); // M2 Task 0.2

        // M2 Task 0.2: Show View Report link for pending, processing, and completed runs
        ...
        // Show Trace Log link if trace mode is enabled
        $rundata->trace_enabled = $trace_enabled;
        if ($trace_enabled) {
            $rundata->trace_url = new moodle_url('/local/customerintel/trace_viewer.php', ['runid' => $run->id]);
        }
