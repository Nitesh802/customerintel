<?php
/**
 * Delete the canonical_nb_dataset artifact for Run 192
 * This will force synthesis to rebuild it with the FIXED canonical_builder code
 */
...
$PAGE->set_url(new moodle_url('/local/customerintel/delete_canonical_artifact.php'));
$PAGE->set_title("Delete Canonical Dataset Artifact");
...
<h1>🗑️ Delete Canonical Dataset Artifact - Run 192</h1>

<div class="section warning">
<h2>⚠️ Why Delete This Artifact?</h2>
<p>The <code>canonical_nb_dataset</code> artifact was created BEFORE we fixed the NB code mismatch bug in canonical_builder.php.</p>
<p>Even though we fixed the code, synthesis is loading the OLD cached artifact instead of rebuilding with the new code.</p>
<p><strong>Solution:</strong> Delete the old artifact to force it to rebuild with 254 citations!</p>
</div>
...
if ($confirm !== 'yes') {
    ...
    echo "<h2>Canonical Dataset Artifact</h2>";
    ...
    if (!$artifact) {
        echo "<p class='warning'>No canonical_nb_dataset artifact found. It may have already been deleted.</p>";
        ...
        $citations_count = 0;
        if ($data) {
            if (isset($data['aggregated_citations'])) {
                $citations_count = count($data['aggregated_citations']);
            } else if (isset($data['processing_stats']['total_citations'])) {
                $citations_count = $data['processing_stats']['total_citations'];
            }
        }

        echo "<tr><td style='padding: 8px; border: 1px solid #dee2e6;'><strong>Citations</strong></td><td style='padding: 8px; border: 1px solid #dee2e6;'>" . ($citations_count > 0 ? $citations_count : "<span style='color: #dc3545;'>0</span>") . "</td></tr>";

        echo "</table>";

        echo "<div class='section warning'>";
        echo "<h3>🔴 This artifact was created with the OLD buggy code!</h3>";
        echo "<p>The canonical dataset has <strong>{$citations_count} citations</strong> (should be 254).</p>";
        echo "<p>Even though we fixed the code, synthesis loads this OLD artifact instead of rebuilding.</p>";
        echo "</div>";
    }
    ...
    if ($artifact) {
        ...
        echo "<p>After deletion, regenerate synthesis to create a NEW artifact with 254 citations!</p>";
    ...
    if ($deleted) {
        ...
        echo "<p><strong>The old canonical_nb_dataset artifact has been removed.</strong></p>";
        echo "</div>";
    } else {
    ...
    echo "<p>This will force canonical_builder to rebuild the artifact using the FIXED code with 254 citations!</p>";
    echo "</div>";
}
