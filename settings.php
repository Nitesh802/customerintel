...
if ($hassiteconfig) {
    ...
    $settings->add(new admin_setting_configselect(
        'local_customerintel/llm_provider',
        get_string('llmprovider', 'local_customerintel'),
        get_string('llmprovider_help', 'local_customerintel'),
        'openai-gpt4',
        [
    ...
    $settings->add(new admin_setting_configtext(
        'local_customerintel/llm_key',
        get_string('llmkey', 'local_customerintel'),
        get_string('llmkey_help', 'local_customerintel'),
        '',
        PARAM_RAW
    ));
    ...
    $settings->add(new admin_setting_heading(
        'local_customerintel/diagnosticssettings',
        'Enhanced Diagnostics & Run Doctor',
        'Advanced pipeline health monitoring and automated diagnostic capabilities'
    ));
    
    // Enable Run Doctor
    $settings->add(new admin_setting_configcheckbox(
        'local_customerintel/enable_run_doctor',
        'Enable Run Doctor',
        'Enable automated diagnostic analysis after each synthesis run. Checks artifacts, phase coverage, and performance anomalies.',
        1
    ));
    
    // Auto-run Diagnostics
    $settings->add(new admin_setting_configcheckbox(
        'local_customerintel/auto_run_diagnostics',
        'Auto-run Diagnostics',
        'Automatically run diagnostic analysis after synthesis completion. Results are stored and viewable in the Diagnostics Hub.',
        1
    ));
    
    // Diagnostic Data Retention
    $settings->add(new admin_setting_configselect(
        'local_customerintel/diagnostic_retention_days',
        'Diagnostic Data Retention (days)',
        'How long to keep diagnostic and telemetry data before automatic cleanup',
        90,
        [
            '30' => '30 days',
            '60' => '60 days',
    ...
    // Diagnostics & Health Hub link
    $ADMIN->add('localplugins', 
        new admin_externalpage(
            'local_customerintel_diagnostics',
            get_string('diagnostics_hub', 'local_customerintel'),
            new moodle_url('/local/customerintel/diagnostics.php'),
