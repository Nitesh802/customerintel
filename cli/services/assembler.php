...
class assembler {
    ...
    protected function map_to_phases(array $nbresults): array {
        ...
        foreach ($nbresults as $nbresult) {
            ...
            if (isset($nbmapping[$normalizedcode])) {
                ...
                $item = [
                    'id' => $normalizedcode,
                    'title' => $mapping['title'],
                    'prompt' => $this->get_prompt_for_nb($normalizedcode),
                    'response' => $this->format_nb_response($nbresult),
                    'citations' => $nbresult->citations ?? [],
                    'status' => $nbresult->status,
                    'hascitations' => !empty($nbresult->citations)
                ];
        ...
        foreach ($phases as &$phase) {
            $phase['itemcount'] = count($phase['items']);
        }
