...
class voice_enforcer {
     ...
     * - At least 1 casual aside (e.g., "honestly...", "frankly...", "look...")
    ...
    public function enforce(string $text): array {
        ...
        // Check 1: Casual aside requirement
        $aside_check = $this->check_casual_asides($text);
        $report['checks']['casual_asides'] = $aside_check;
        
        if (!$aside_check['passed']) {
            $text = $this->add_casual_aside($text);
            $report['rewrites_applied'][] = 'Added casual aside';
        }
    ...
    private function add_casual_aside(string $text): string {
        $asides = ['Honestly', 'Look', 'Frankly'];
        $aside = $asides[array_rand($asides)];
        
        // Find first paragraph or sentence to add aside
        if (preg_match('/^([^.!?]*[.!?])/', $text, $matches)) {
            $first_sentence = $matches[1];
            $replacement = $aside . ', ' . lcfirst(trim($first_sentence));
            return str_replace($first_sentence, $replacement, $text);
        }
        
        return $aside . ', ' . lcfirst($text);
    }
